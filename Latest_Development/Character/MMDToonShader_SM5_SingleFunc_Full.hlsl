// MMD Toon Shader - UE5 Custom Node 完整整合版
// ================================================================
// 本文件是 MMDToonShader_SM5_SingleFunc.hlsl 的完整整合版：
// 以示例材质 M_Redner_Master1 为准，把前一项目删改掉的功能补回，
// 同时保留 SingleFunc 的色调三模式/饱和度/HSV 统一调色等增强能力。
//
// 整合自示例材质的新功能：
//   1. 曲线阴影映射（UseCurve 开关，默认开启）——采样 CurveLinearColorAtlas
//      曲线图集，曲线可实时调整无需重画贴图；关闭时回退 Ramp 贴图/三层色带
//   2. ShadowSmooth / ShadowLocation —— 阴影平滑度与位置（示例材质的
//      NdotL/ShadowSmooth - ShadowLocation 重映射）
//   3. 双 Matcap —— 高光 Matcap + 粗糙 Matcap（RoughMatcapTexture），
//      解决单一 Matcap 无法区分光滑/哑光区域的问题
//   4. HM 贴图（R=高光蒙版，G=AO）—— 用一张贴图同时控制高光范围和
//      环境光遮蔽，代替单一参数
//   5. AO 独立控制（AO_Power_ShadowMask / AO_Shadow_Strengh）
//   6. SSS 次表面散射（SSSTex）—— 皮肤边缘透光
