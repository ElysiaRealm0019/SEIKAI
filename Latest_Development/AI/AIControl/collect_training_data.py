...
N_NORMALS = 2048  # 球面法线采样数（只被 diag_ex_fallback.py 用，采样管线已不再需要）
...
#   逐帧跳 0.5° 以上的帧占 14%（_exp/diag_sweep_flicker.py）。MLP 无论容量多大都
#   只是把这堆矛盾以有限保真度复刻下来。
#
...
#   是逐参数调 shader 时唯一能在 Python 侧预判画面亮度的工具 —— 见 diag_ex_fallback.py。
...
def resolve_params(L: np.ndarray):
    ...
    **三个输出都是纯标签场的值，没有任何合成规则改写。** 这里曾对 EX 做「死黑兜底」
    ...
    理由见本文件上方 _IDW_POWER 下面的长注释和 diag_ex_fallback.py ——
