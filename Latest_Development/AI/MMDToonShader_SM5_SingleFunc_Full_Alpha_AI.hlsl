...
// AI 驱动版（2026-09-12）
// ================================================================
// 本文件是 MMDToonShader_SM5_SingleFunc_Full_Alpha.hlsl 的 AI 集成变体：
// 与 AI/MMDToonShader_SM5_SingleFunc_Full_AI.hlsl 同源同构，差别只在本文件输出
// float4(RGB, Alpha)、且多了 Alpha 透明度处理段。
// 内嵌 MLP（3→256→3）按 LdotV / L_up / L_right 预测 ShadowSmooth /
// ShadowLocation / ExposureScale；UseAI=1 时用预测值覆盖手动参数，=0 时全部走手动。
// ⚠ MLP 权重块由 retrain_all.py 写进 Full_AI.hlsl。本文件是同一份权重的副本，
//    重训后必须同步更新，否则不透明版与透明版会静默漂移成两套参数。
// ================================================================
// ================================================================
...
//                                       ShadowLocation/ExposureScale，0=全部手动）
// ----------------------------------------------------------------
